...
const PhotoGallery = () => {
  ...
  const showVerifyModal = () => {
    ...
    document.getElementById("preview").innerHTML = title;
  };
  ...
  const onUpload = async () => {
    try {
      ...
      const storyResponse = await axios.post(`${API_URL}/galery`, {
        title,
        images: imageResponses.map((response) => response.data.image),
      });

      message.success("Your Photo Galery was successfully uploaded");
      setTitle("");
      setImages([]);
      ...
      message.error("Error uploading story. Please try again.");
    }
  };
